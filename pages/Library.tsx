...
const Library: React.FC = () => {
  ...
  // 1. Lấy danh sách các dân tộc DUY NHẤT có trong dữ liệu để tạo bộ lọc
  ...
  const filteredData = useMemo(() => {
    return libraryData.filter(item => {
      ...
      const matchSearch = item.ethnic.toLowerCase().includes(searchTerm.toLowerCase()) || 
  ...
  return (
    <div className="relative min-h-screen font-display bg-[#F7F3E9]">
      ...
      <div className="max-w-[1920px] mx-auto px-4 lg:px-6 py-8 lg:py-12 relative z-10">
        <header className="mb-8 lg:mb-12 text-center">
          <div className="flex items-center justify-center gap-4 text-primary font-bold tracking-[0.3em] uppercase text-xs lg:text-sm mb-4">
            <span className="h-px w-8 lg:w-12 bg-primary"></span>
            Kho Tàng Di Sản Số
            <span className="h-px w-8 lg:w-12 bg-primary"></span>
          </div>
          <h2 className="text-4xl lg:text-7xl font-black text-text-main italic mb-6">
            Tiếng Vọng <span className="text-gold">Tiền Nhân</span>
          </h2>
          ...
          <div className="max-w-md mx-auto relative group z-20 px-4 lg:px-0">
            <input 
              ...
              className="w-full bg-white/80 backdrop-blur-sm border-2 border-gold/20 rounded-full py-3 px-6 pl-12 text-text-main placeholder:text-text-soft/50 focus:outline-none focus:border-primary transition-colors shadow-lg text-sm"
            />
        ...
        {/* --- MOBILE FILTER TOOLBAR (Hiển thị trên mobile/tablet) --- */}
        <div className="lg:hidden mb-8 space-y-4 sticky top-20 z-30 bg-[#F7F3E9]/95 backdrop-blur-md py-2 -mx-4 px-4 border-b border-gold/10 shadow-sm">
              ...
              {categories.map((cat) => (
                 <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-full whitespace-nowrap transition-all border ${
                      ...
                      : 'bg-white border-gold/20 text-text-soft hover:bg-gold/10'
                    }`}
                 >
           ...
           {/* 2. Ethnic Filter Horizontal */}
           <div className="flex gap-2 overflow-x-auto custom-scrollbar-h pb-2" ref={scrollRef}>
              {availableEthnics.map((ethnic) => (
                 <button
                   key={ethnic}
                   onClick={() => setSelectedEthnicFilter(ethnic)}
                   className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider whitespace-nowrap border transition-all ${
                     selectedEthnicFilter === ethnic
                     ? 'bg-text-main border-text-main text-gold'
                     : 'bg-transparent border-gold/20 text-text-soft hover:border-gold'
                   }`}
                 >
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          {/* LEFT SIDEBAR: FILTERS (Chỉ hiện trên Desktop) */}
          <aside className="hidden lg:block w-72 shrink-0 space-y-8 sticky top-24 z-10">
            ...
            <div className="bg-white p-2 rounded-[2rem] border border-gold/20 shadow-xl">
               {categories.map((cat) => (
                 <button
                    key={cat.id}
                    onClick={() => { setActiveCategory(cat.id as Category); setSelectedEthnicFilter('ALL'); }}
                    className={`w-full flex items-center gap-4 p-4 rounded-[1.5rem] transition-all duration-300 mb-2 last:mb-0 ${
                      activeCategory === cat.id 
                      ? 'bg-primary text-white shadow-lg' 
                      : 'hover:bg-gold/10 text-text-soft'
                    }`}
                 >
                    <span className={`material-symbols-outlined ${activeCategory === cat.id ? 'text-white' : 'text-primary'}`}>{cat.icon}</span>
                    <span className="font-black uppercase tracking-widest text-xs">{cat.label}</span>
                    {activeCategory === cat.id && <span className="ml-auto material-symbols-outlined text-sm">chevron_right</span>}
                 </button>
               ))}
            </div>
            ...
            <div className="bg-white rounded-[2rem] border border-gold/20 shadow-xl overflow-hidden flex flex-col max-h-[60vh]">
               <div className="p-5 border-b border-gold/10 bg-gold/5">
                 <h3 className="font-black uppercase text-xs tracking-[0.2em] text-text-main flex items-center gap-2">
               ...
               <div className="overflow-y-auto p-2 custom-scrollbar flex-1">
                 {availableEthnics.map((ethnic) => (
                   <button
                     key={ethnic}
                     onClick={() => setSelectedEthnicFilter(ethnic)}
                     className={`w-full text-left px-5 py-3 rounded-xl text-sm font-bold transition-all mb-1 flex justify-between items-center ${
                       selectedEthnicFilter === ethnic
                       ? 'bg-text-main text-gold'
          ...
          <main className="flex-1 w-full">
             <div className="mb-6 hidden lg:flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                   <h3 className="text-2xl font-black text-text-main uppercase italic">
                     {activeCategory === 'architecture' && 'Không gian kiến trúc'}
                     {activeCategory === 'ritual' && 'Nghi lễ & Tín ngưỡng'}
                     {activeCategory === 'festival' && 'Lễ hội & Văn hóa'}
                   </h3>
                   {selectedEthnicFilter !== 'ALL' && (
                     <span className="bg-primary text-white px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest">
                       {selectedEthnicFilter}
                     </span>
                   )}
                </div>
                <div className="flex items-center gap-4 text-xs font-bold text-bronze uppercase tracking-widest">
            ...
            {currentData.length > 0 ? (
              <>
                {/* 
                   CẤU HÌNH LƯỚI (RESPONSIVE GRID):
                   - Mobile (Dọc): grid-cols-2 (2 cột để dễ nhìn hơn 3 cột trên màn hình nhỏ)
                   - Tablet/Mobile Ngang: md:grid-cols-3 (3 cột)
                   - Desktop/Ngang rộng: lg:grid-cols-4 (4 cột)
                */}
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
                  {currentData.map((item) => (
                    <div 
                      ...
                      className="group bg-white rounded-2xl md:rounded-[2rem] overflow-hidden border border-gold/15 shadow-md hover:shadow-2xl hover:-translate-y-2 active:scale-[0.98] transition-all duration-300 cursor-pointer relative flex flex-col h-[300px] md:h-[400px]"
                    >
                      <div className="h-40 md:h-56 overflow-hidden relative shrink-0">
                        <img 
                          ...
                          decoding="async" 
                        />
                        <div className="absolute top-2 left-2 md:top-4 md:left-4 bg-primary/90 text-white text-[8px] md:text-[9px] font-black px-2 md:px-3 py-1 md:py-1.5 rounded-full uppercase tracking-widest backdrop-blur-md border border-white/20 shadow-md z-10">
                          {item.ethnic}
                        </div>
                      </div>
                      ...
                      <div className="p-3 md:p-6 flex flex-col flex-grow">
                        ...
                        <div className="flex items-center justify-between border-t border-gold/10 pt-2 md:pt-4 mt-auto">
                          ...
                          <div className="size-6 md:size-8 rounded-full bg-gold/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300 group-hover:rotate-45">
                             <span className="material-symbols-outlined text-xs md:text-lg">arrow_outward</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {totalPages > 1 && (
                  <div className="mt-8 md:mt-12 flex items-center justify-center gap-2 pb-20 lg:pb-0">
                    <button 
                      ...
                      className="size-8 md:size-10 rounded-full flex items-center justify-center border border-gold/20 bg-white text-primary disabled:opacity-30 disabled:cursor-not-allowed hover:bg-primary hover:text-white transition-colors"
                    >
                      ...
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => {
                         ...
                         return (
                          <button
                            ...
                            className={`size-8 md:size-10 rounded-full flex items-center justify-center text-[10px] md:text-xs font-black transition-all ${
                              currentPage === page 
                              ? 'bg-primary text-white shadow-lg scale-110' 
                              : 'bg-white border border-gold/10 text-text-soft hover:border-gold/50'
                            }`}
                          >
                            {page}
                          </button>
              ...
              <div className="text-center py-32 bg-white/50 rounded-[3rem] border-2 border-dashed border-gold/20">
                 <span className="material-symbols-outlined text-6xl text-text-soft/20 mb-4">manage_search</span>
                 <p className="text-text-soft font-bold text-lg">Chưa có dữ liệu cho lựa chọn này.</p>
                 <button onClick={() => setSelectedEthnicFilter('ALL')} className="mt-4 text-primary font-black uppercase text-xs hover:underline">Xem tất cả dân tộc</button>
              </div>
            )}
          </main>
        </div>
        ...
        {selectedItem && createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 font-display">
            {/* Backdrop */}
            <div 
              className="absolute inset-0 bg-text-main/80 backdrop-blur-md animate-fade-in" 
              onClick={() => setSelectedItem(null)}
            ></div>
            
            {/* Modal Container */}
            <div className="bg-white w-[95vw] h-[85vh] rounded-2xl md:rounded-[2.5rem] shadow-2xl relative z-10 animate-slide-up flex overflow-hidden border-4 border-gold/20 flex-col md:flex-row">
               
               <button 
                  onClick={() => setSelectedItem(null)}
                  className="absolute top-4 right-4 md:top-6 md:right-6 z-50 size-10 md:size-12 rounded-full bg-white/20 hover:bg-primary hover:text-white transition-all flex items-center justify-center text-white backdrop-blur-md border border-white/30 hover:scale-110 active:scale-90 group"
               >
                 <span className="material-symbols-outlined text-xl md:text-2xl group-hover:rotate-90 transition-transform">close</span>
               </button>

               {/* Cột Ảnh */}
               <div className="w-full md:w-[70%] h-2/5 md:h-full relative shrink-0 group bg-[#F2EFE6] border-b md:border-b-0 md:border-r border-gold/10">
                 ...
                 <div className="absolute bottom-4 left-4 md:bottom-10 md:left-16 text-white z-20 max-w-2xl animate-slide-up-content p-2">
                    <div className="inline-block px-3 py-1 md:px-4 md:py-1.5 bg-gold text-text-main text-[8px] md:text-[10px] font-black uppercase tracking-widest rounded-full mb-2 md:mb-4 shadow-lg border border-white/20">
                       Dân tộc {selectedItem.ethnic}
                    </div>
                    <h2 className="text-2xl md:text-4xl lg:text-6xl font-black italic leading-tight mb-2 md:mb-4 drop-shadow-lg">{selectedItem.title}</h2>
                    <div className="w-12 md:w-20 h-1 md:h-1.5 bg-primary rounded-full"></div>
                 </div>
               </div>

               {/* Cột Nội dung */}
               <div className="w-full md:w-[30%] h-3/5 md:h-full p-6 md:p-8 lg:p-10 bg-white relative flex flex-col">
                 <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none">
                    <span className="material-symbols-outlined text-8xl md:text-[10rem] text-primary">
                      {selectedItem.category === 'architecture' ? 'temple_hindu' : selectedItem.category === 'ritual' ? 'self_improvement' : 'festival'}
                    </span>
                 </div>
                 ...
                 <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 animate-fade-in delay-100 pr-2">
                    <div className="flex items-center gap-3 mb-4 md:mb-6">
                       ...
                       <span className="text-xs font-black text-primary uppercase tracking-widest">
                         Chi tiết {selectedItem.category === 'architecture' ? 'Kiến trúc' : selectedItem.category === 'ritual' ? 'Nghi lễ' : 'Lễ hội'}
                       </span>
                    </div>

                    <div className="prose prose-sm md:prose-lg text-text-main font-serif leading-relaxed text-justify">
                       <p className="font-bold text-base md:text-xl italic text-text-soft/90 mb-4 md:mb-6 border-l-4 border-gold pl-4 py-1">"{selectedItem.desc}"</p>
                       {selectedItem.content.split('\n').map((paragraph, idx) => (
                         <p key={idx} className="mb-3 md:mb-4 text-sm md:text-base">{paragraph}</p>
                       ))}
                    </div>
                 </div>

                 <div className="pt-4 md:pt-6 border-t border-gold/10 flex gap-4 shrink-0 mt-2 md:mt-4">
                        <button className="flex-1 bg-primary text-white py-3 md:py-4 rounded-xl md:rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-gold hover:text-text-main transition-all shadow-xl hover:-translate-y-1">
                           Tìm hiểu thêm
                        </button>
                        <button className="px-4 md:px-6 border-2 border-gold/20 rounded-xl md:rounded-2xl hover:bg-white text-primary transition-colors flex items-center justify-center hover:scale-105">
      ...
      <style>{`
        ...
        /* Ẩn scrollbar ngang cho menu mobile nhưng vẫn scroll được */
...
export const libraryData: LibraryItem[] = [
  ...
  // Khmer
  {
    ...
    image: "pictures-thuvien/nghi-le/khmer/le-dang-y-kathina-khmer.jpg"
  },
  ...
  // Tây Nguyên
  {
    ...
    image: "pictures-thuvien/nghi-le/gia-rai/le-bo-ma-pothi-giarai.jpg"
  },
  {
    ...
    image: "pictures-thuvien/nghi-le/ba-na/le-dam-trau-bana.jpg"
  },
  {
    ...
    image: "pictures-thuvien/nghi-le/e-de/le-cung-ben-nuoc-ede.jpg"
  },
  ...
  // Các dân tộc khác
  {
    ...
    image: "pictures-thuvien/nghi-le/muong/mo-muong.jpg"
  },
  {
  ...
  },
  {
    ...
    image: "pictures-thuvien/nghi-le/odu/le-tieng-sam-odu.jpg"
  },
  {
  ...
  },
  {
    ...
    image: "pictures-thuvien/nghi-le/chut/le-lap-lo-chut.jpg"
  },
  {
  ...
  // Kinh
  {
    ...
    image: "pictures-thuvien/le-hoi/kinh/tet-nguyen-dan-kinh.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/kinh/gio-to-hung-vuong-kinh.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/kinh/hoi-giong-kinh.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/kinh/hoi-lim-kinh.jpg"
  },
  ...
  // Khmer
  {
    ...
    image: "pictures-thuvien/le-hoi/khmer/chol-chnam-thmay-khmer.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/khmer/ok-om-bok-khmer.jpg"
  },
  ...
  // Thái
  {
    ...
    image: "pictures-thuvien/le-hoi/thai/le-hoi-hoa-ban-thai.jpg"
  },
  ...
  // Chăm
  {
    ...
    image: "pictures-thuvien/le-hoi/cham/le-hoi-kate-cham.jpg"
  },
  ...
  // H'Mông, Tày, Dao...
  {
    ...
    image: "pictures-thuvien/le-hoi/hmong/le-hoi-gau-tao-hmong.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/tay/le-hoi-long-tong-tay.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/dao/tet-nhay-dao.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/pa-the-n/le-hoi-nhay-lua-pa-the-n.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/mnong/le-hoi-dua-voi-mnong.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/lolo/le-hoi-cau-mua-lolo.jpg"
  },
  {
    ...
    image: "pictures-thuvien/le-hoi/hanhi/le-hoi-kho-gia-gia-hanhi.jpg"
  }
];
